...
/**
 * #500 — /api/agent402/* must be middleware-allowlisted.
 *
 * The Agent402 proxy self-authenticates exactly like /api/memory/* and
...
describe('middleware /api/agent402 allowlist (#500)', () => {
  beforeEach(() => vi.clearAllMocks())
  afterEach(() => vi.clearAllMocks())

  it('does NOT 401 a tokenless /api/agent402/capabilities request', async () => {
    const res = await middleware(apiReq('/api/agent402/capabilities'))
    expect(res.status).not.toBe(401)
  })

  it('does NOT 401 a tokenless /api/agent402/projects request', async () => {
    const res = await middleware(apiReq('/api/agent402/projects'))
    expect(res.status).not.toBe(401)
  })
