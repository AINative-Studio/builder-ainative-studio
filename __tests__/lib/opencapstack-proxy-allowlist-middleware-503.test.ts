...
/**
 * #503 — /api/opencapstack/* must be middleware-allowlisted.
 *
 * The OpenCapStack proxy self-authenticates exactly like /api/memory/* and
 * /api/agent402/*: the same signed per-app data token, verified inside the
 * route handler. A generated app has no next-auth session at all.
 */
...
describe('middleware /api/opencapstack allowlist (#503)', () => {
  beforeEach(() => vi.clearAllMocks())
  afterEach(() => vi.clearAllMocks())

  it('does NOT 401 a tokenless /api/opencapstack/company request', async () => {
    const res = await middleware(apiReq('/api/opencapstack/company'))
