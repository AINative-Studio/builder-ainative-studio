...
/**
 * #505 — /api/model-catalog/* must be middleware-allowlisted.
 *
 * The Model Catalog proxy self-authenticates exactly like /api/agent402/*,
 * /api/memory/*, and /api/db/*: the same signed per-app data token, verified
 * inside the route handler. A generated app has no next-auth session at all.
 */
...
describe('middleware /api/model-catalog allowlist (#505)', () => {
  beforeEach(() => vi.clearAllMocks())
  afterEach(() => vi.clearAllMocks())

  it('does NOT 401 a tokenless /api/model-catalog/list request', async () => {
    const res = await middleware(apiReq('/api/model-catalog/list'))
