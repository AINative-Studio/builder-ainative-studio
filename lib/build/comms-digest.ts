...
export function buildAgileDigest(input: AgileDigestInput): AgileDigest {
  ...
  if (!input.dailyReportContent) {
    ...
    return {
      subject,
      text,
      html: `<p>Good morning,</p><p>Cody has no updates to report for <strong>${escapeHtml(company)}</strong> this run — the nightly loop hasn't produced a report yet, or there was nothing new overnight.</p><p>— Cody</p>`,
    }
  }
...
export function buildPairProgrammingDigest(input: PairProgrammingDigestInput): PairProgrammingDigest {
  ...
  if (input.commits.length === 0) {
    ...
    return {
      ...
      text,
      html: `<p>Hi,</p><p>No new commits on <strong>${escapeHtml(company)}</strong>'s repo since ${escapeHtml(input.sinceIso)} — Cody hasn't pushed any changes in this window.</p><p>— Cody</p>`,
    }
  }
