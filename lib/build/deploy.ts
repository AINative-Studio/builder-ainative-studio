 ...
 * *.ainative.app wildcard host is configured (AINATIVE_WILDCARD_HOST), a company
 * gets a REAL dedicated, DNS-pointable host at {slug}.ainative.app — served by the
 * Builder itself via host→slug rewriting in middleware (no per-company service to
 * provision). #240's DNS can then CNAME a custom domain → {slug}.ainative.app
...
/** The shared wildcard apex, e.g. "ainative.app". When set, companies get a real
 ...
 * isn't a company subdomain (apex, www, a multi-label subdomain, a different host,
 * or no wildcard configured). Pure + host-only (port stripped) so middleware can
 * rewrite {slug}.ainative.app → /build/{slug} and this stays unit-testable.
 */
export function wildcardSlugFromHost(host: string | null, wildcardHost = WILDCARD_HOST): string | null {
  ...
  const suffix = `.${wildcardHost}`
  if (!h.endsWith(suffix)) return null
  const sub = h.slice(0, -suffix.length)
  // Must be a single-label, non-www subdomain matching the slug charset.
  if (!sub || sub === 'www' || sub.includes('.') || !/^[a-z0-9_-]+$/.test(sub)) return null
  return sub
}
 ...
 * https://{slug}.ainative.app — a dedicated, DNS-pointable address served by the
 ...
 * is true, so #240 can CNAME a custom domain → {slug}.ainative.app.
 *
...
export async function deployPersistent(
  ...
  // --- Real persistent host: shared *.ainative.app wildcard --------------
