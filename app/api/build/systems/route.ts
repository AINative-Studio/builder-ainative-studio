...
export async function GET(request: NextRequest) {
  ...
  // Best-effort: read the company's real counts from its ZeroDB tables if the
  // enrolled company has any. Failures / no-data → honest zero-state.
  let counts: Partial<Record<BusinessSystem["key"], { count?: number; value?: number }>> = {}
  if (companyId) {
    try {
  ...
  return Response.json({ systems: buildSystems(counts), companyId, zeroState: Object.keys(counts).length === 0 })
}
